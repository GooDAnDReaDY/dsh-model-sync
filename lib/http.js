...
export function isLoopbackAddress(ip) {
  ...
  if (clean.startsWith('127.') || clean.startsWith('::ffff:127.')) return true
  return false
}

export function trusted(req, { requireLoopback = false } = {}) {
  const remoteAddress = req?.socket?.remoteAddress
  if (requireLoopback || req?.method === 'POST') {
  ...
  if (origin) {
    ...
    try {
      ...
      if (requireLoopback || req?.method === 'POST') {
        if (!isLoopbackAddress(url.hostname) && url.hostname !== 'localhost') return false
      }
    } catch { return false }
  }
  ...
  // If sec-fetch-site is same-origin or same-site, allow
  if (fetchSite === 'same-origin' || fetchSite === 'same-site') return true
  // If sec-fetch-mode is present but origin/referer missing on a cross-site request, reject
  if (req.headers?.['sec-fetch-mode'] && !host) return false
  // For POST write requests without any browser origin/referer headers, require explicit loopback remoteAddress
  if (req?.method === 'POST' && !origin && !referer && !isLoopbackAddress(remoteAddress)) return false
  return true
}
...
export function normalizeTryRequest(body) {
  if (body === null || typeof body !== 'object' || Array.isArray(body)) throw new Error('request body must be an object')
  if (typeof body.provider !== 'string' || !body.provider) throw new Error('provider is required')
  if (typeof body.model !== 'string' || !body.model) throw new Error('model is required')
  return { provider: body.provider, model: body.model }
}


export function normalizeBatchTryRequest(body) {
  if (body === null || typeof body !== 'object' || Array.isArray(body)) throw new Error('request body must be an object')
  if (typeof body.provider !== 'string' || !body.provider) throw new Error('provider is required')
  if (!Array.isArray(body.models)) throw new Error('models must be an array')
  return {
    provider: body.provider.trim(),
    models: body.models.filter((m) => typeof m === 'string' && m.trim()).map((m) => m.trim()),
  }
}

export function normalizeAliasRequest(body) {
  if (body === null || typeof body !== 'object' || Array.isArray(body)) throw new Error('request body must be an object')
  if (typeof body.alias !== 'string' || !body.alias.trim()) throw new Error('alias is required')
  if (!/^[a-zA-Z0-9_\-.]{1,64}$/.test(body.alias.trim())) throw new Error('alias must be 1-64 alphanumeric characters or _-.')
  if (body.action === 'delete') return { action: 'delete', alias: body.alias.trim() }
  if (typeof body.provider !== 'string' || !body.provider.trim() || body.provider.length > 128) throw new Error('provider must be a short string')
  if (typeof body.model !== 'string' || !body.model.trim() || body.model.length > 256) throw new Error('model must be a short string')
  return { action: 'set', alias: body.alias.trim(), provider: body.provider.trim(), model: body.model.trim() }
}
...
export function normalizePolicyRequest(body) {
  ...
  return {
    ...
    policy: validatePolicy({
      ...
      enableCostFilter: body.enableCostFilter,
      maxPricePerMillion: body.maxPricePerMillion,
      enableContextFilter: body.enableContextFilter,
      minContextTokens: body.minContextTokens,
    }),
  }
}
...
export function registerHttpApi(ctx, synchronizer, { getSchedulerStatus = () => null } = {}) {
  const makeHandler = (route) => async (req, res) => {
    if (!trusted(req, { requireLoopback: req.method === 'POST' })) return json(res, 403, { error: { code: 'forbidden', message: 'dsh-model-sync: cross-origin or non-loopback request rejected' } })
    if (route === 'status') {
      if (req.method !== 'GET') return json(res, 405, { error: { code: 'method', message: 'GET only' } })
      const payload = { ...synchronizer.status(), scheduler: getSchedulerStatus(), providers: synchronizer.listProviders() }
      const bodyStr = JSON.stringify(payload)
      const etag = 'W/"' + crypto.createHash('sha1').update(bodyStr).digest('hex').slice(0, 16) + '"'
    ...
    if (route === 'export') {
      if (req.method !== 'GET') return json(res, 405, { error: { code: 'method', message: 'GET only' } })
      return json(res, 200, synchronizer.exportConfig())
    }
    if (route === 'aliases' && req.method === 'GET') {
      return json(res, 200, { aliases: synchronizer.getAliases() })
    }
    if (route === 'import') {
      if (req.method !== 'POST') return json(res, 405, { error: { code: 'method', message: 'POST only' } })
      try {
        const body = await readJson(req)
        const result = await synchronizer.importConfig(body)
        return json(res, 200, result)
      } catch (error) {
        const code = error?.code === 'INVALID_CONFIG' || error?.code === 'INVALID_POLICY' ? 400 : 500
        return json(res, code, { error: { code: error?.code ?? 'import-failed', message: error instanceof Error ? error.message : String(error) } })
      }
    }
    if (route === 'report') {
      if (req.method !== 'GET') return json(res, 405, { error: { code: 'method', message: 'GET only' } })
    ...
    try {
      ...
      options = route === 'selection'
        ...
        : route === 'try' ? normalizeTryRequest(body)
        : route === 'batch-try' ? normalizeBatchTryRequest(body)
        : route === 'aliases' ? normalizeAliasRequest(body)
    ...
    try {
      const result = route === 'selection'
        ...
        : route === 'try' ? await synchronizer.tryModel(options)
        : route === 'batch-try' ? await synchronizer.batchTryModels(options)
        : route === 'aliases' ? (options.action === 'delete' ? await synchronizer.deleteAlias(options) : await synchronizer.setAlias(options))
      ...
      const code = error?.code === 'SETTINGS_CONFLICT' ? 409 : error?.code === 'SETTINGS_UNAVAILABLE' || error?.code === 'CONFIG_UNAVAILABLE' ? 503 : error?.code === 'PROVIDER_NOT_CONFIGURED' || error?.code === 'UNKNOWN_MODEL' || error?.code === 'INVALID_POLICY' || error?.code === 'INVALID_HISTORY' || error?.code === 'HISTORY_NOT_FOUND' || error?.code === 'INVALID_NOTIFICATION' || error?.code === 'NOTIFICATION_NOT_FOUND' || error?.code === 'INVALID_ALIAS' || error?.code === 'INVALID_CONFIG' ? 400 : 500
  ...
  return [
    ...
    ctx.webServer.register({ kind: 'exact', path: `${BASE}/try`, handler: makeHandler('try') }),
    ctx.webServer.register({ kind: 'exact', path: `${BASE}/batch-try`, handler: makeHandler('batch-try') }),
    ctx.webServer.register({ kind: 'exact', path: `${BASE}/export`, handler: makeHandler('export') }),
    ctx.webServer.register({ kind: 'exact', path: `${BASE}/import`, handler: makeHandler('import') }),
    ctx.webServer.register({ kind: 'exact', path: `${BASE}/aliases`, handler: makeHandler('aliases') }),
