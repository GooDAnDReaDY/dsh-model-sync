...
export function registerHttpApi(ctx, synchronizer, { getSchedulerStatus = () => null } = {}) {
  const makeHandler = (route) => async (req, res) => {
    ...
    try {
      ...
      const code = error?.code === 'SETTINGS_CONFLICT' ? 409 : error?.code === 'SETTINGS_UNAVAILABLE' || error?.code === 'CONFIG_UNAVAILABLE' ? 503 : error?.code === 'PROVIDER_NOT_CONFIGURED' || error?.code === 'UNKNOWN_MODEL' || error?.code === 'INVALID_POLICY' || error?.code === 'INVALID_HISTORY' || error?.code === 'HISTORY_NOT_FOUND' ? 400 : 500
