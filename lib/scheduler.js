...
export function createSyncScheduler(sync, {
  ...
  async function tick() {
    ...
    try {
      for (const row of due) {
        ...
        const options = { dryRun: config.autoApply === true ? false : true, reportSource: 'schedule' }
  ...
  function start() {
    ...
    const isFirstRun = state.size === 0 || [...state.values()].every((row) => row.lastRunAt === null)
    resetStates(config)
    ...
    if (isFirstRun) {
      Promise.resolve().then(() => tick()).catch((err) => {
        logger.warn?.("[dsh-model-sync] initial tick failed: " + errorMessage(err))
      })
    }
    return true
  }
  ...
  function status() {
    ...
    const nextAt = (timer !== null && values.length > 0) ? Math.min(...values.map((row) => row.nextAt)) : null
    ...
    return {
      ...
      lastRunAt,
      nextRunAt: nextAt,
      providers: Object.fromEntries(values.map((row) => [row.provider ?? '*', { lastRunAt: row.lastRunAt, nextRunAt: timer !== null ? row.nextAt : null, lastStatus: row.lastStatus, ...(row.lastError ? { lastError: row.lastError } : {}) }])),
    }
  }
