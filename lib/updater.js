...
/**
 * Host-side one-click updater for @goodandready/dsh-model-sync.
 * Implements the unified DSH plugin updater specification with loopback verification.
 */
...
function findDshCliEntry() {
  ...
  const resolved = entry.startsWith('file:') ? fileURLToPath(entry) : resolve(process.cwd(), entry)
    ...
    if (existsSync(manifestPath)) {
      try {
        ...
        if (
          ...
          typeof bin === 'string' &&
          !isAbsolute(bin) &&
          resolve(directory, bin) === resolve(resolved)
        ) {
          return resolved
        }
      } catch (err) {
        // bestEffort manifest parse
      }
    }
...
export function resetLatestCache() {
...
export async function status(options, target) {
  ...
  if (typeof options.getLatestVersion === 'function') {
    latest = await options.getLatestVersion()
  } else if (options.latestVersion !== undefined) {
    latest = options.latestVersion
  } else {
    latest = await latestVersion(options.packageName, options.registry ?? 'https://registry.npmjs.org')
  }
  return {
...
export function registerPluginUpdater(ctx, options = {}) {
  ...
  if (typeof ctx?.webServer?.register !== 'function') {
    if (typeof ctx?.logger?.warn === 'function') {
      ctx.logger.warn('[dsh-model-sync] webServer service unavailable, updater skipped')
    }
    return () => {}
  }

  const packageName = options.packageName ?? '@goodandready/dsh-model-sync'
  const endpoints = Array.isArray(options.endpoints)
    ? options.endpoints
    : [options.endpoint ?? '/api/dsh-model-sync/update', '/dsh-model-sync/update']
  ...
  const handler = async (request, response) => {
    try {
      ...
      if (request.method === 'GET' || request.method === 'HEAD') {
        const payload = await status({ ...options, packageName }, target)
      ...
      installing = true
      try {
        const before = await status({ ...options, packageName }, target)
        ...
        const doInstall = options.installExact ?? installExact
        await doInstall(target, `${packageName}@${before.latestVersion}`, options)
        json(response, 200, {
          ...before,
      ...
      if (typeof ctx?.logger?.warn === 'function') {
        ctx.logger.warn(`[dsh-model-sync] plugin updater failed: ${String(error)}`)
      }
      json(response, 503, { error: 'Plugin update failed; see server logs.' })
    }
  }

  const disposers = uniqueEndpoints.map((path) =>
  ...
  return () => {
    for (const dispose of disposers) {
      if (typeof dispose === 'function') {
        try { dispose() } catch (err) { /* bestEffort cleanup */ }
      }
    }
  }
}
