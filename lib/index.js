...
export function apply(ctx, config) {
  ...
  const bindSettings = (settings, ownerCtx) => {
    ...
    saveConfigImpl = (patch) => {
      const nextTask = saveQueue.then(async () => {
        const result = await scope.update(patch)
        liveConfig = resolveConfig(scope.get() || baseConfig)
        return result
      })
      saveQueue = nextTask.catch(() => {})
      return nextTask
    }
    ownerCtx.effect(() => () => {
  ...
  let settings
  try { settings = ctx.get('settings') } catch (e) {
    if (typeof ctx.logger?.debug === 'function') ctx.logger.debug('[dsh-model-sync] settings service not immediately available:', e)
  }
  if (!bindSettings(settings, ctx)) {
    ctx.inject(['settings'], (sctx) => {
      bindSettings((typeof sctx.get === 'function' ? sctx.get('settings') : null) ?? sctx.settings, sctx)
    })
  }
