window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = '.dms-card{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;list-style:none;margin:0;padding:0;overflow:hidden;box-sizing:border-box}' +
      ...
      '.dms-card-header:hover{background:var(--dsw-alias-interactive-bg-hover,var(--dsw-alias-bg-layer-2))}' +
      ...
      '.dms-toolbar,.dms-section{display:flex;flex-direction:row;align-items:center;flex-wrap:wrap;gap:10px;padding:13px 14px;border:1px solid var(--dsw-alias-border-l2,var(--dsw-alias-border-primary));border-radius:10px;background:var(--dsw-alias-bg-layer-1,transparent);box-sizing:border-box}' +
      ...
      '.dms-wrap>.dms-actions{padding:10px 14px;border:1px solid var(--dsw-alias-border-l2,var(--dsw-alias-border-primary));border-radius:10px;background:var(--dsw-alias-bg-layer-1,transparent)}' +
      ...
      '.dms-btn:hover:not(:disabled){border-color:var(--dsw-alias-brand-primary);background:var(--dsw-alias-bg-layer-2)}' +
      ...
      '.dms-badge-ok{border-color:var(--dsw-alias-state-success-primary);color:var(--dsw-alias-state-success-primary);background:var(--dsw-alias-state-success-secondary,var(--dsw-alias-bg-layer-2))}' +
      '.dms-badge-warn{border-color:var(--dsw-alias-state-warning-primary);color:var(--dsw-alias-state-warning-primary);background:var(--dsw-alias-state-warning-secondary,var(--dsw-alias-bg-layer-2))}' +
      '.dms-badge-bad{border-color:var(--dsw-alias-state-error-primary);color:var(--dsw-alias-state-error-primary);background:var(--dsw-alias-state-error-secondary,var(--dsw-alias-bg-layer-2))}' +
      ...
      '.dms-btn-danger{color:var(--dsw-alias-state-error-primary)!important;border-color:var(--dsw-alias-state-error-primary)!important}' +
      '.dms-btn-danger:hover:not(:disabled){background:var(--dsw-alias-state-error-secondary,var(--dsw-alias-bg-layer-2))!important;border-color:var(--dsw-alias-state-error-primary)!important}' +
      '.dms-alert{padding:10px 14px;border-radius:8px;font-size:13px}' +
      '.dms-alert-ok{background:var(--dsw-alias-state-success-secondary,var(--dsw-alias-bg-layer-2));color:var(--dsw-alias-state-success-primary)}' +
      '.dms-alert-bad{background:var(--dsw-alias-state-error-secondary,var(--dsw-alias-bg-layer-2));color:var(--dsw-alias-state-error-primary)}' +
    ...
    const PKG = '@goodandready/dsh-model-sync'
    const ROW_ID = 'dsh-model-sync'
    ...
    function UpdaterSection({ t, api }) {
      const [updaterStatus, setUpdaterStatus] = React.useState(null)
      const [updaterChecking, setUpdaterChecking] = React.useState(false)
      const [updaterUpdating, setUpdaterUpdating] = React.useState(false)
      const [updaterNotice, setUpdaterNotice] = React.useState('')
      const [updaterError, setUpdaterError] = React.useState('')

      const checkUpdates = () => {
        setUpdaterChecking(true)
        setUpdaterError('')
        setUpdaterNotice('')
        fetch(`${api}/update?fresh=1`)
          .then((res) => {
          ...
          .catch(() => {
            setUpdaterChecking(false)
            setUpdaterError(t.checkUpdateFailed)
          })
      }

      const runUpdate = () => {
        if (updaterUpdating) return
        ...
        fetch(`${api}/update`, {
          ...
          .then((resData) => {
            ...
            const cur = resData.updatedVersion || (updaterStatus && updaterStatus.latestVersion) || ''
            const nextData = Object.assign({}, updaterStatus, { currentVersion: cur, updateAvailable: false })
            setUpdaterUpdating(false)
            setUpdaterStatus(nextData)
            setUpdaterNotice(t.updateSuccess + cur + (resData.restartRequired ? t.restartRequired : ''))
          })
          .catch((err) => {
            setUpdaterUpdating(false)
            setUpdaterError(t.updateFailed + (err.message || String(err)))
          })
      }

      return React.createElement('div', { style: { marginTop: 8 } },
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between' } },
          React.createElement('div', { style: { fontSize: 13, color: 'var(--dsw-alias-label-primary)' } },
            updaterStatus ? `v${updaterStatus.currentVersion}` : '',
            updaterStatus && updaterStatus.updateAvailable ? ` → v${updaterStatus.latestVersion}` : ''
          ),
          React.createElement('div', null,
            updaterStatus && updaterStatus.updateAvailable
              ? React.createElement('button', {
                  ...
                  onClick: runUpdate,
                }, updaterUpdating ? t.updating : t.updateNow)
              : React.createElement('button', {
                  type: 'button',
                  className: 'dms-btn',
                  disabled: updaterChecking,
                  onClick: checkUpdates,
                }, updaterChecking ? t.checkingUpdates : (updaterStatus ? t.upToDate : t.checkForUpdates))
          )
        ),
        updaterNotice ? React.createElement('div', { style: { color: 'var(--dsw-alias-state-success)', marginTop: 8, fontSize: 12 } }, updaterNotice) : null,
        updaterError ? React.createElement('div', { style: { color: 'var(--dsw-alias-state-error-primary)', marginTop: 8, fontSize: 12 } }, updaterError) : null
      )
    }
    ...
    function ModelSyncSection({ ctx, includeIntro = true }) {
      ...
      const scope = React.useMemo(() => (ctx && ctx.configForms ? ctx.configForms.get(NS) : undefined), [ctx])
      ...
      return React.createElement('div', { className: 'dms-wrap' },
        ...
        React.createElement('div', { className: 'dms-section-card dms-section' },
          React.createElement('div', { className: 'dms-section-title' }, React.createElement('strong', null, t.updaterTitle)),
          React.createElement(UpdaterSection, { t, api: API })
        )
      )
    }

    function ModelSyncCard({ ctx, view }) {
      const locale = useLocale(ctx)
      const t = locale === 'zh' ? zh : en
      ...
      if (view === 'summary') {
        return React.createElement('span', { className: 'dms-card-description' }, t.desc)
      }
      return React.createElement(page ? 'div' : 'li', { className: page ? 'dms-page' : 'dms-card' + (open ? ' dms-card-open' : '') },
        React.createElement('button', { type: 'button', className: 'dms-card-header', style: page ? { display: 'none' } : undefined, 'aria-expanded': page ? true : open, 'aria-label': `${t[open ? 'hideSettings' : 'showSettings']}: ${t.title}`, onClick: () => setOpen((value) => !value) },
        ...
        (page || open) ? React.createElement('div', { className: 'dms-card-body' }, React.createElement(ErrorBoundary, null, React.createElement(ModelSyncSection, { ctx, includeIntro: false }))) : null
      )
    }

    function apply(ctx) {
      ctx.effect(() => ctx.locale.register(NS, { en, zh }), 'dsh-model-sync: dictionaries')
      try {
        // Plugin-list seat (plugins.item) first: it is the seat the current core
        ...
        ctx.slots.inject('plugins.item', () => ctx.slots.register({ name: 'plugins.item', id: ROW_ID, order: 60, label: () => 'Model synchronization', locale: NS, inject: () => ({ ctx }) }, (props) => React.createElement(ErrorBoundary, null, React.createElement(ModelSyncCard, { ...props, ctx }))))
        // Row seat and the legacy seat stay as fallbacks.
        ctx.slots.inject('plugins.row.config', () => ctx.slots.register({ name: 'plugins.row.config', key: ROW_CONFIG_KEY, locale: NS, inject: () => ({ ctx }) }, (props) => React.createElement(ErrorBoundary, null, React.createElement(ModelSyncCard, { ...props, ctx }))))
    ...
    module.exports = { apply, inject: ['slots', 'locale', 'configForms'], filterModelsByCapabilities }
