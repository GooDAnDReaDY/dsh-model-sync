window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ModelSyncSection({ ctx }) {
      ...
      const rows = (status?.providers ?? []).filter((row) => row.configured); const button = (label, onClick) => React.createElement('button', { type: 'button', disabled: busy, onClick, style: { marginRight: 8 } }, label)
