...
const PROVIDER_ADAPTERS = Object.freeze({
  ...
  'kimi-coding': { endpoint: 'https://api.kimi.com/coding/v1/models', auth: 'bearer' },
...
async function requestSpecific(profile, descriptor, { resolveCredential, fetchImpl, signal }) {
  ...
  if (!response.ok) {
    const error = catalogRequestError(response.status)
    error.adapterCode = [401, 403].includes(response.status) ? 'auth' : 'http'
    throw error
  }
  return response
}
...
async function fetchSpecific(profile, descriptor, options) {
  ...
  return parseRows(profile.provider, payload, descriptor)
}
