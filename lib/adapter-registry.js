...
import { normalizeModels, parseClinePassModels } from './models.js'

const MAX_RESPONSE_BYTES = 4 * 1024 * 1024
const SAFE_AUTH = new Set(['bearer', 'x-api-key', 'query-key', 'none'])
const SAFE_PARSERS = new Set(['openai', 'google', 'clinebot-plan'])
...
const PROVIDER_ADAPTERS = Object.freeze({
  clinebot: {
    endpoint: 'https://api.cline.bot/api/v1/users/me/plan',
    auth: 'bearer',
    parser: 'clinebot-plan',
    defaultModels: [
...
function parseRows(provider, payload, descriptor) {
  if (descriptor.parse === 'clinebot-plan') {
    const plan = payload?.data?.plan || payload?.data || payload?.plan || payload
    const included = plan?.features?.included || plan?.includedModels || []
    return parseClinePassModels(included)
  }
...
export function createProviderAdapterRegistry({ resolveCredential, fetchImpl = fetch, adapters = {} } = {}) {
  ...
  const registry = {
    ...
    select(profile, config) {
      ...
      if (config && config.enabled !== false && config.endpoint) return declarative(profile, config)
      ...
      if (config && config.enabled !== false && profile?.baseURL) return declarative(profile, config)
