...
import {
  ...
  replaceWithRetry,
  rotationRefs,
} from './synchronizer-helpers.js'
import { createConfigTransfer } from './synchronizer-transfer.js'
import { createModelProber } from './synchronizer-prober.js'
...
export function createModelSynchronizer(ctx, { getConfig = () => ({}), saveConfig = async () => {}, fetchImpl = fetch, adapterImplementations = {} } = {}) {
  ...
  const providerHttpMeta = new Map()
  ...
  function providers() {
    ...
    return listApiKeyProviders(llmOf(ctx), settingsOf(ctx)).map((row) => {
  ...
  async function discover(row, { signal, reliability = DEFAULT_RELIABILITY } = {}) {
    ...
    let retries = 0
    const existingMeta = providerHttpMeta.get(row.provider)
    try {
      const result = await retryWithBackoff(async () => {
        const timed = createTimeoutSignal(signal, reliability.timeoutMs)
        try {
          return await adapter.discover(profile, { signal: timed.signal, etag: existingMeta?.etag, lastModified: existingMeta?.lastModified })
        } finally {
          timed.cleanup()
        }
      }, {
      ...
      if (result && result.notModified) {
        const config = getConfig() ?? {}
        const cached = normalizeModels(row.provider, config.modelCatalogs?.[row.provider] ?? row.models ?? [])
        return { provider: row.provider, status: 'ok', models: cached, retries, notModified: true }
      }
      const models = Array.isArray(result) ? result : []
      if (result?.__httpMeta) {
        providerHttpMeta.set(row.provider, result.__httpMeta)
      }
  ...
  async function run(options = {}) {
    ...
    const task = (async () => {
      ...
      if (resultsToApply.length > 0) {
        const settings = settingsOf(ctx)
        const piaiRows = resultsToApply.filter((r) => r.provider !== 'commandcode')
        const ccRows = resultsToApply.filter((r) => r.provider === 'commandcode')
        if (piaiRows.length > 0) {
          await replaceWithRetry(settings, PIAI_NS, (section) => {
            let next = section
            for (const row of piaiRows) {
              next = catalogPatch(next, row.provider, row.next)
            }
            return next
          })
        }
        for (const row of ccRows) {
          if (typeof settings?.mutate === 'function') {
            await settings.mutate('llm-commandcode', [
              { op: 'set', path: ['visibleModels'], value: row.selectedModels?.length ? row.selectedModels : row.next.map(m => m.id) }
            ])
          }
        }
        result.applied = true
      ...
      if (!dryRun || resultsToApply.length > 0) {
        ...
        if (results.some((row) => row.changed) || Object.keys(previousCatalogs).length !== Object.keys(cachedCatalogs).length || JSON.stringify(previousCatalogs) !== JSON.stringify(cachedCatalogs)) patch.modelCatalogs = cachedCatalogs
  ...
  const prober = createModelProber({ health })
  const tryModel = prober.tryModel
  const batchTryModels = prober.batchTryModels
  ...
  async function rollbackHistory({ historyId, provider } = {}) {
    ...
    await replaceWithRetry(settings, PIAI_NS, (section) => catalogPatch(section, provider, target))
  ...
  async function setModelSelection(provider, models) {
    ...
    // Persist the allowlist first.
    await saveConfig({ modelSelections })
    try {
      if (provider === 'commandcode') {
        if (typeof settings?.mutate === 'function') {
          await settings.mutate('llm-commandcode', [
            { op: 'set', path: ['visibleModels'], value: selectedModels }
          ])
        }
        if (section?.providers?.commandcode && typeof settings?.replace === 'function') {
          await replaceWithRetry(settings, PIAI_NS, (sec) => catalogPatch(sec, provider, nextModels))
        }
      } else {
        if (!section || !descriptor || typeof settings.replace !== 'function') {
          throw Object.assign(new Error('llm-pi-ai settings are not writable'), { code: 'SETTINGS_UNAVAILABLE' })
  ...
  const transfer = createConfigTransfer({
    getConfig,
    saveConfig,
    listProviders: providers,
    validatePolicy,
  })
  const exportConfig = transfer.exportConfig
  const importConfig = transfer.importConfig
  const getAliases = transfer.getAliases
  const setAlias = transfer.setAlias
  const deleteAlias = transfer.deleteAlias
  ...
  return Object.freeze({
    ...
    rollbackHistory,
    status: () => ({ running: Boolean(running), lastRun, lastHealth, lastReport: structuredClone(lastReport), notifications: notificationLedger(), history: history(), policies: listPolicies(), lifecycle: structuredClone(getConfig()?.modelLifecycle ?? {}), aliases: structuredClone(getConfig()?.aliases ?? {}) }),
  })
}
