...
export function createModelSynchronizer(ctx, { getConfig = () => ({}), saveConfig = async () => {}, fetchImpl = fetch } = {}) {
  ...
  async function discover(row, { signal, reliability = DEFAULT_RELIABILITY } = {}) {
    ...
    try {
      ...
      recordCredentialOutcome(row.provider, { status: 'error', code: statusOfError(error), retryable: isRetryableError(error), message: diagnosticMessage(error), retries })
      return {
        provider: row.provider,
        status: 'error',
        models: [],
  ...
  return Object.freeze({
    ...
    history,
    rollbackHistory,
    status: () => ({ running: Boolean(running), lastRun, lastHealth, history: history(), policies: listPolicies(), lifecycle: structuredClone(getConfig()?.modelLifecycle ?? {}) }),
  })
}
