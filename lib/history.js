...
function cloneModel(provider, model) {
  const normalized = normalizeModels(provider, [model])[0]
  ...
  if (normalized.capabilities) compact.capabilities = normalized.capabilities
  if (normalized.pricing) compact.pricing = normalized.pricing
  if (normalized.contextWindow !== undefined) compact.contextWindow = normalized.contextWindow
  if (normalized.maxTokens !== undefined) compact.maxTokens = normalized.maxTokens
  return compact
}

function catalog(provider, models) {
  return (normalizeModels(provider, models) ?? []).map((model) => cloneModel(provider, model)).filter(Boolean).slice(0, MAX_HISTORY_MODELS)
}
