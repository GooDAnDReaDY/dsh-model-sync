...
export async function retryWithBackoff(operation, {
  ...
  for (let attempt = 1; attempt <= total; attempt += 1) {
    ...
    try {
      ...
      const exponential = Math.max(0, Number(baseDelayMs) || 0) * (2 ** (attempt - 1))
      // Full jitter: randomized between [0.5, 1.0] of calculated exponential backoff
      const jitterFactor = jitter ? (0.5 + Math.random() * 0.5) : 1
      const calculatedDelayMs = Math.min(Math.max(0, Number(maxDelayMs) || 0), Math.round(exponential * jitterFactor))
