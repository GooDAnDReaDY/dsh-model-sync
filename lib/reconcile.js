import { diffModels, normalizeModels, stableStringify } from './models.js'
  ...
  for (const row of before) {
    ...
    if (fresh) {
      const merged = {
        ...
        ...(row.compat || fresh.compat ? {
          compat: { ...(fresh.compat ?? {}), ...(row.compat ?? {}) },
        } : {}),
        ...(fresh.input?.length ? { input: fresh.input } : row.input ? { input: row.input } : {}),
      }
      next.push(merged)
